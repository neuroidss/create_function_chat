...
tools_global=[
      {
        ...
        'function': {
          'name': 'create_tool',
          'description': 'tool to create tool',
          'parameters': {
            'type': 'object',
            'properties': {
              'new_tool_name': {
            ...
            'required': ['new_tool_name', 'new_tool_description'],
          },
        },
      },
    ]
...
def create_tool(**data) -> str:
  name = data['new_tool_name']
  description = data['new_tool_description']
...
#  messages = [{'role': 'user', 'content': 'Write python async function '+name+' which makes: "'+description+'", and next write in json format which parameters was used to call that function if any. '}]
  ...
  function =  {
          ...
          'description': 'place function description here',
          'parameters': {
            'type': 'object',
            'properties': {
...
#  messages_global.append(response['message'])
  ...
  if response['message'].get('tool_calls'):
    for tool in response['message']['tool_calls']:
      if tool['function']['name']=='create_tool':
...
#            run('qwen2.5:7b-instruct-q8_0', message)
    #        run('qwen2.5-coder:7b-instruct-q8_0', message)
            ...
            run('MFDoom/deepseek-r1-tool-calling:14b', message)
#            run('MFDoom/deepseek-r1-tool-calling:8b', message)
#            run('MFDoom/deepseek-r1-tool-calling:7b-qwen-distill-q8_0', message)
#            run('MFDoom/deepseek-r1-tool-calling:7b', message)
#            run('MFDoom/deepseek-r1-tool-calling:1.5b-qwen-distill-q8_0', message)
#            run('MFDoom/deepseek-r1-tool-calling:1.5b', message)
#            run('x/llama3.2-vision', message)
